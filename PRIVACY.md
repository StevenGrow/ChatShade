...
These settings are saved with `chrome.storage.sync`, which is Chrome's extension storage system. If Chrome Sync is enabled for the user's Chrome profile, Chrome may sync these settings between Chrome installations signed in to that profile.
